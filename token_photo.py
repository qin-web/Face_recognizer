...
class win_1(QDialog):
    ...
    def initUI(self):
        ...
        self.btnQuit = QPushButton('Retake', self)
        self.label = QLabel()

        # 布局设定
        layout = QGridLayout(self)
        layout.addWidget(self.label, 0, 1, 3, 3)
        # layout.addWidget(self.btnOpen, 4, 1, 1, 1)
        layout.addWidget(self.btnSave, 3, 1, 1, 1)
        layout.addWidget(self.btnProcess, 3, 2, 1, 1)
        layout.addWidget(self.btnQuit, 3, 3, 1, 1)

        # 信号与槽进行连接，信号可绑定普通成员函数
        # self.btnOpen.clicked.connect(self.refreshShow)
        self.btnSave.clicked.connect(self.saveSlot)
        self.btnProcess.clicked.connect(self.processSlot)
        self.btnQuit.clicked.connect(self.close)

        self.img = cv2.imread('/home/wjc/Documents/code/face_test/1.jpg', -1)
        if self.img.size == 1:
            return
